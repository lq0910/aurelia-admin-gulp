...
export class Addcommodity {
    ...
    onSubmit() {
        ...
        this.commodityservice.add_commodity(data).then(
            os => {
                if (os.ok) {
                    console.log("添加商品成功！", os);
                    // this.resshop = os.payload
                    this.layer.msg('添加成功')
                    setTimeout(() => {
                        this.router.navigate('commodity_list', { refresh: true })
    ...
    getList() {
        ...
        this.from_service.list_all1().then(
            os => {
                if (os.ok) {
