...
export class Addshop {
    ...
    attached() {
        ...
        console.log('dddddddddd', this.shop);
    ...
    onSubmitShop() {
        ...
        this.shopservice.add_shop(data).then(
            os => {
                if (os.ok) {
                    console.log("添加店铺成功！", os);
        ...
        layui.use('upload', function () {
            ...
            that.uploadInst = upload.render({
                ...
                , done: function (res) {
                    ...
                    that.percent = '0%'
                    that.element.progress('progress-shop', '0%'); //可配合 layui 进度条元素使用
                    that.showProgress = false //隐藏进度条
                }
                , choose: function (obj) {
                    //将每次选择的文件追加到文件队列
                    var files = obj.pushFile();
                    console.log('files', files);
                    that.showProgress = true //显示进度条
                ...
                progress: function (n) {
                    ...
                    that.element.progress('progress-shop', percent); //可配合 layui 进度条元素使用
                }
                , error: function (err) {
                    //请求异常回调
